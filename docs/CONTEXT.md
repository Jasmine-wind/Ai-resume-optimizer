...
当前 ResumeEditor 已将既有 Section 排序收敛为无常驻 handle 的桌面直接操作：在非交互区域按住鼠标约 220ms 后移动超过 6px 才进入拖动，松开后只通过既有 document change → dirty → autosave → CAS 链路保存；textarea、input、button、summary 等交互控件不会启动排序。拖动态使用轻量 picked-up surface 和插入线，靠近编辑区上下边缘时缓慢自动滚动；Section 获得焦点后可用 Alt + 上 / 下方向键排序，并通过 aria-live 报告位置。当前 Entry / Bullet 没有既有排序入口，本轮未新增其排序能力；Evidence anchor 仍以稳定 ID 保持不变。
...
当前事实（manual acceptance corrective）：仅 Classic 的当前模板由 v3 升为 v4，新增 `typst/classic/v4/main.typ`；Classic v3 及更早版本保留供历史导出物解释，Modern / Minimal 继续使用 v3。Classic v4 只调整排版呈现：统一章节节奏、紧凑的国内技术岗单栏结构、Experience / Project 的标题—角色—日期组合、Education / Summary / Skills / Certificate / Achievement / Other 的语义化紧凑展示；不改变 `RESUME_DOCUMENT_V1`、渲染器版本 `typst-resume-renderer/3`、Preview receipt 或 Export Gate。
