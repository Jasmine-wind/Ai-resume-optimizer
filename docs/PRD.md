...
这只描述材料的支持强度，不代表“用户现实中确实有该经历但没写清楚”。Rewrite 可以把岗位要求作为候选补充方向，但必须明确提示当前材料未体现，不能把它伪装成 Evidence 或已确认事实。
...
这不代表用户没有相应能力。AI Rewrite 可以参考该要求提出候选补充，但必须明确标记“当前材料未体现”，并由用户在查看 Diff 后自行确认；Evidence 仍然保持 NO_EVIDENCE，不会被改写。
...
AI 可以提出当前 Bullet 没写明的技术、职责、成果、数字或岗位能力候选，输入可以包括用户要求、岗位要求和 Evidence 分析。候选不会自动写入简历；用户必须看到原文、建议版本、Diff 和具体 reason，并通过显式 Apply 对最终真实性负责。

如果建议包含原文未出现或变化较大的信息，系统提供 review advisory（例如“请确认内容真实”），但不会因为内容事实变化隐藏建议、REJECT 或禁止 Apply。reason 必须具体指出补充或变化了什么，并提醒用户核对真实性；不要鼓励无依据的编造。

以下仍然不是可用候选：malformed JSON、Provider failure、timeout、空或超长输出、内部元素 ID / UUID 泄露、不可见控制字符、结构不安全输出，以及 stale revision / hash mismatch。这些技术与并发安全问题继续 fail closed。
...
账号菜单属于账号区域，不新增顶部一级导航；当前最小设置入口为：
...
账户设置内按类别导航，当前只提供“个人资料”和“AI 设置”。个人资料仅允许编辑显示名称（nickname）；用户名和邮箱作为账户标识暂不修改。
...
- AI Suggest 可以提出未在当前材料出现的候选，但不自动写入；用户通过显式 Apply 确认最终内容
- 对原文未出现或变化较大的信息提供 review advisory，而不是事实变化硬拦截
- Evidence 继续诚实区分“当前材料足够支持 / 只有部分证据 / 当前材料未体现”，不因 Rewrite 候选改变
- 不把当前材料之外的能力或经历伪装成 Evidence 或系统确认事实
